...
def compute_targets(anchor, cls, bbox):
    ...
    # gt_bboxes = anchor
    # B, A, _ = anchor.shape
    # gt_clss = torch.tensor(np.zeros((B, A, 1)))
    # for i in range(B):
    #     for j in range(A):
    #         iou = compute_bbox_iou( anchor[i], gt_bboxes[i],dim=1)
    #         if(iou < 0.4):
    #             gt_clss[B][A] = 0
    #             gt_bboxes[B][A] = 0
    #        elif(iou >= 0.5):
    #             gt_clss[B][A] = cls[B][A]
    #             gt_bboxes[B][A] = bbox[B][A]
    #        else:
    #             gt_clss[B][A] = -1
    #             gt_bboxes[B][A] = 0
...
def apply_bbox_deltas(boxes, deltas):
    ...
    # new_x1 = torch.zeros((N,), dtype=torch.int)
    # new_y1 = torch.zeros((N,), dtype=torch.int)
    # new_x2 = torch.zeros((N,), dtype=torch.int)
    # new_y2 = torch.zeros((N,), dtype=torch.int)
    #for i in range(N):
...
def nms(bboxes, scores, threshold=0.5):
    ...
    #     for j in range(N):
    #         if i != j:
    #             ious[i][j] = (compute_bbox_iou(bboxes[i].unsqueeze(0), bboxes[j].unsqueeze(0)))
    # n = np.sum(np.abs(ious), axis=1)
